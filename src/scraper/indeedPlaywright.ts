...
export class IndeedPlaywrightScraper {
  private baseUrl = 'https://in.indeed.com/jobs?q=software+engineer&l=India&sort=date';

  async scrape(limit = 10): Promise<RawJob[]> {
    console.log(`[Indeed] Starting scrape for India software jobs...`);
    ...
    try {
      ...
      for (const el of jobElements.slice(0, limit)) {
        try {
          // Based on debug HTML:
          // Title: h3.jobTitle a span[title]
