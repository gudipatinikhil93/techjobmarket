...
const $$Search = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": `Search Results for "${q}" | JobMarketIndia` }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "Navbar", $$Navbar, {})} ${maybeRenderHead()}<main class="py-24"> <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"> <h1 class="text-3xl font-bold text-white mb-8">Search Results for "${q}"</h1> ${results.length > 0 ? renderTemplate`<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> ${results.map((role) => renderTemplate`<a${addAttribute(`/role/${encodeURIComponent(role.role)}`, "href")} class="rounded-xl border border-white/10 bg-card p-6 hover:border-brand-cyan/30 transition-all group"> <div class="flex justify-between items-start mb-4"> <span${addAttribute(`text-xs font-bold uppercase ${statusColors[role.status]}`, "class")}> ${role.status} </span> <span class="text-xs text-muted-foreground">${role.job_count} jobs</span> </div> <h3 class="text-xl font-bold text-white mb-4 group-hover:text-brand-cyan transition-colors">${role.role}</h3> <p class="text-sm text-muted-foreground mb-4">
Market Avg: ₹${Math.round(role.market_avg_min / 1e5)}L - ₹${Math.round(role.market_avg_max / 1e5)}L
